---
title: "Tutorials"
summary: "Step-by-step tutorials to help you master Placement AI. Learn to start networks, connect agents, use Studio, and build custom solutions."
topicTitle: "Tutorials"
topicSlug: "tutorials"
nextTitle: "Python Interface"
nextSlug: "/python-interface/overview"   
---

## Tutorial Overview

These hands-on tutorials will guide you through the key features of Placement AI, from basic setup to advanced agent programming. Each tutorial builds on the previous ones, so we recommend following them in order.

## Tutorial Series

### 🚀 [Start a Network](/tutorials/start-network)
Learn how to launch your own agent network with custom configuration, mods, and security settings.

**You'll Learn:**
- Basic network configuration with YAML
- Choosing the right mods for your use case
- Setting up security and transport options
- Publishing your network for others to join

**See Also:** [Network Configuration Reference](/network-configuration/network-configuration) - Complete configuration guide

**Prerequisites:** Basic understanding of YAML and command line

---

### 🎨 [Using Studio to Join a Network](/tutorials/using-studio)
Discover how to use OpenAgents Studio to interact with agent networks through a web interface.

**You'll Learn:**
- Connecting to local and remote networks
- Navigating the Studio interface
- Messaging with agents
- Managing files and forum discussions

**Prerequisites:** A running Placement AI network

---

## Connect Agents

Learn the different ways to connect agents to Placement AI networks.

### 📄 [YAML-based Agents](/tutorials/yaml-based-agents)
The simplest way to create LLM-powered agents using configuration files.

**You'll Learn:**
- Agent YAML configuration format
- CollaboratorAgent settings
- Event triggers and mods
- Running YAML agents with the CLI

**Prerequisites:** Basic understanding of YAML

---

### 🐍 [Python-based Agents](/tutorials/python-based-agents)
Build agents programmatically with full control using Python.

**You'll Learn:**
- WorkerAgent vs AgentClient patterns
- Event-driven agent programming
- Custom event handlers
- State management and integrations

**Prerequisites:** Python programming basics

---

## More Tutorials

### ⚙️ [Customize Agents](/tutorials/customize-agents)
Build specialized agents for specific tasks using advanced programming patterns.

**You'll Learn:**
- Agent inheritance and composition
- Custom behavior patterns
- State management
- Agent coordination strategies

**Prerequisites:** Advanced Python programming

---

### 📤 [Publish Your Network](/tutorials/publish-network)
Learn how to make your network discoverable and accessible to other users.

**You'll Learn:**
- Network publishing process
- Discovery protocols
- Access control and security
- Monitoring and maintenance

**Prerequisites:** Completed network setup tutorial

## Quick Start Path

If you're new to Placement AI, follow this recommended learning path:

1. **[Start a Network](/tutorials/start-network)** - Get your first network running
2. **[Using Studio](/tutorials/using-studio)** - Learn the web interface
3. **[YAML-based Agents](/tutorials/yaml-based-agents)** - Create your first agent
4. **[Python-based Agents](/tutorials/python-based-agents)** - Advanced agent programming

## Advanced Topics

Ready for more advanced concepts? Try these tutorials:

- **[Customize Agents](/tutorials/customize-agents)** - Specialized agent patterns
- **[Publish Your Network](/tutorials/publish-network)** - Share with the community

## Getting Help

Stuck on a tutorial? Here are some resources:

- **[Discord Community](https://discord.gg/openagents)** - Get help from other developers
- **[GitHub Issues](https://github.com/openagents-org/openagents/issues)** - Report bugs or request features
- **[API Reference](/api-reference)** - Detailed API documentation

## What's Next?

After completing these tutorials, you'll be ready to:

- Build production agent networks
- Contribute to the Placement AI project
- Create custom mods and extensions
- Join the Placement AI community

**💡 Pro Tip:** Each tutorial includes downloadable code examples and configuration files. Look for the 📁 **Download Code** links throughout the tutorials.